package org.darklol9.bots.utils.asm.analysis.stack;

import org.darklol9.bots.utils.asm.Casts;
import org.darklol9.bots.utils.asm.analysis.SuperInterpreter;
...
public class ConstantTracker extends Interpreter<ConstantValue> implements Opcodes {
    ...
    private Object getUnaryValue(int opcode, Object value) {
        ...
        switch (opcode) {
            case INEG:
                return -((int) numVal.intValue());
            case FNEG:
                return -((float) numVal.floatValue());
            case LNEG:
                return -((long) numVal.longValue());
            case DNEG:
                return -((double) numVal.doubleValue());
            case L2I:
            case F2I:
            case D2I:
    ...
    public ConstantValue binaryOperation(AbstractInsnNode insn, ConstantValue a, ConstantValue b)
        ...
        switch (insn.getOpcode()) {
            ...
            case LALOAD:
            case AALOAD:
                if (a.isCertain() && a.isKnown() && b.isCertain() && b.isKnown() && b.isInteger()) {
